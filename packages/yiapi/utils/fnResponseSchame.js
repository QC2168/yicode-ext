...
export function fnResponseSchema(opt) {
  const {
    ...
    hasToken = (schema !== null) ? DEFAULT_CONFIG.hasToken : false
  } = { ...DEFAULT_CONFIG, ...opt };
  // 子数据
  const properties = schema ? Object.entries(schema).filter(i => !extract.includes(i[0])).reduce((pre, cur) => ({ ...pre, [cur[0]]: fnSchema(cur[1]) }), {}) : {}
  return {
    type: 'object',
    properties: {
      symbol: {
